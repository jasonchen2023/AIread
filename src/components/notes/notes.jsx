...
import { getUserNote, pushUserNote } from '../../services/firebase';
...
function Notes(props) {
  ...
  useEffect(() => {
    const fetchInitialNote = async () => {
      try {
        const initialNote = await getUserNote(id, props.chunkNum, errorGetCallback);
  ...
  const onSaveClick = () => {
    console.log(text);
    setIsEditing(false);
    pushUserNote(id, text, props.chunkNum, successAddCallback, errorAddCallback);
  };
